...
function refresh_access_token() {
  ...
  response="$(curl -sS --stderr - -F "client_id=$imgur_acct_key" -F "client_secret=$imgur_secret" -F "grant_type=refresh_token" -F "refresh_token=$refresh_token" https://api.imgur.com/oauth2/token)"
  if [ ! "$?" -eq "0" ]; then
    # curl failed
...
function fetch_account_info() {
  response="$(curl -sS --stderr - -H "Authorization: Bearer $access_token" https://api.imgur.com/3/account/me.xml)"
...
function upload_authenticated_image() {
  echo "Uploading '$1'..."
  response="$(curl --connect-timeout "$upload_connect_timeout" -m "$upload_timeout" --retry "$upload_retries" -sS --stderr - -F "image=@$1" -H "Authorization: Bearer $access_token" https://api.imgur.com/3/image.xml)"
...
function upload_anonymous_image() {
  echo "Uploading '$1'..."
  response="$(curl --connect-timeout "$upload_connect_timeout" -m "$upload_timeout" --retry "$upload_retries" -sS --stderr - -F "image=@$1" -F "key=$imgur_anon_key" https://imgur.com/api/upload.xml)"
